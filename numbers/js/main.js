...
$('.calendar').append(' <div class="calendar__select"> <div class="calendar__select--active day">01</div><ul class="calendar__select--list"></ul> </div><div class="calendar__select"> <div class="calendar__select--active mounth">01</div><ul class="calendar__select--list"></ul> </div><div class="calendar__select"> <div class="calendar__select--active year">2018</div><ul class="calendar__select--list"></ul> </div>')
$('.calendar').each(function(index, el) {
    $(el).find('.calendar__select--list').each(function(index_list, el_list) {
    	let num = index_list
    	if(num==0){
    		for (var i = 0; i < 31; i++) {
    			let val = i;
...
$('.calendar__select--active').on('click',function(e){
	let select = $(this).parents('.calendar__select'),
		list = select.find('ul'),
		active = $(this);
		if(!select.hasClass('active')){
			let active_select = $('.calendar__select.active'),
			list_active = active_select.find('ul')
			list_active.slideUp()
			active_select.removeClass('active')
		}
...
$('.sendForm').on('click',function(e){
	...
	let form = $(this).parents('form'),
		day = form.find('.day').text(),
		mounth = form.find('.mounth').text(),
		year = form.find('.year').text(),
		email = form.find('input[type=email]'),
 		name = form.find('input[type=text]'),
		date = form.find('input[name=date]'),
        dayInput = form.find('input[name=day]'),
        mounthInput = form.find('input[name=mounth]'),
        yearInput = form.find('input[name=year]');


		let date_val = day+'.'+mounth+'.'+year;

		date.val(date_val);
        dayInput.val(day);
        mounthInput.val(mounth);
        yearInput.val(year);
...
$('.notAll__video iframe').each(function(index, el) {
    let iframeWidth = $(el).width(),
        iframeHeight = iframeWidth * 0.56;
    $(el).attr('style', 'height:' + iframeHeight + 'px')
});
